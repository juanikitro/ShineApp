...
test('DashboardAnalyticsPanel provides a section map without hiding the analytical reading', () => {
	...
	const pulsePanel = document.getElementById('dashboard-analysis-pulse')
	const capacityPanel = document.getElementById('dashboard-analysis-capacity')
	...
	assert.ok(pulsePanel?.textContent?.includes('Facturación'))
	assert.ok(pulsePanel?.textContent?.includes('Caja real'))
	assert.ok(capacityPanel?.textContent?.includes('Lavadero'))
	assert.ok(screen.getByRole('heading', { name: 'Margen por servicio' }))
