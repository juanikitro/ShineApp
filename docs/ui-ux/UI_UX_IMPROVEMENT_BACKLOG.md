...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | UI-009 | P1/P2 | Arquitectura frontend | Continuar extraccion por vertical de `frontend/app/page.tsx` sin mezclar redisenos. | Cortes aplicados: Caja en `components/cash/CashPanel.tsx`, Deudas en `components/debts/DebtPanel.tsx`, Dashboard en `components/dashboard/DashboardPanel.tsx`, Configuracion > Negocio en `components/settings/BusinessSettingsPanel.tsx` y el resto de Configuracion en `components/settings/SettingsWorkspace.tsx`; `page.tsx` sigue grande con 16.040 lineas y aprox. 158 `render*`. | Menos riesgo de regresion y cambios UI mas faciles de validar; siguiente corte recomendado: Inventario o Herramientas. |
...
| UI-009 | Caja + Deudas + Dashboard + Settings completo / arquitectura frontend | Parcialmente reducido: renders de Caja, Deudas, Dashboard y Configuracion completa extraidos a `CashPanel.tsx`, `DebtPanel.tsx`, `DashboardPanel.tsx`, `BusinessSettingsPanel.tsx` y `SettingsWorkspace.tsx`; estado, callbacks, payloads y reglas siguen en `page.tsx`. |
...
| --- | --- | --- | --- |
| UI-009 | P1/P2 | Arquitectura frontend | Cortes aplicados en Caja, Deudas, Dashboard y Settings completo; quedan otras verticales, estado, formularios y calculos todavia concentrados en `page.tsx`. |
