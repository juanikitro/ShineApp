...
| --- | --- | --- | --- | --- | --- |
| UI-009 | P1/P2 | App shell / frontend | Reducido a ~85% completo: Caja, Deudas, Dashboard, Configuracion completa, Inventario y Herramientas ya no viven como bloques JSX dentro de `frontend/app/page.tsx`, pero el archivo sigue concentrando Cotizaciones/Servicios, formularios, detalles y calculos. | Conteo post-Herramientas: `frontend/app/page.tsx` tiene 15.024 lineas y aprox. 155 coincidencias `render*`; componentes extraidos: `CashPanel.tsx`, `DebtPanel.tsx`, `DashboardPanel.tsx`, `BusinessSettingsPanel.tsx`, `SettingsWorkspace.tsx`, `InventoryPanel.tsx` y `ToolsPanel.tsx`. | Deuda restante ~15% para UI-009: continuar con Cotizaciones/Servicios; luego evaluar si formularios/detail renders compartibles pertenecen a otro ticket o al remanente tecnico. |
