...
| Portar un overlay manual a `document.body` dentro de un `Dialog` modal de Radix | El dialogo desactiva los punteros fuera de `Dialog.Content`; el overlay queda visible pero no recibe clic | Resolver el contenedor desde el trigger y usar el dialogo mas cercano; conservar `document.body` solo fuera de modales | Test de seleccion por puntero de `SearchSelect` dentro de `ModalFrame` |
