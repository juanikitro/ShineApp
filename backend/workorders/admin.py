...
class WorkOrderAdmin(admin.ModelAdmin):
    list_display = ["id", "customer", "get_status", "service", "received_at", "total_amount"]
    list_filter = ["reservation__status", "received_at"]
    search_fields = ["customer__name", "vehicle__license_plate", "customer__email"]
    list_per_page = 25
    ordering = ["-received_at"]
    readonly_fields = [
        "get_status", "paid_amount", "balance_due", "material_cost",
        "created_at", "updated_at",
    ]
    autocomplete_fields = ["customer", "vehicle", "service"]
    save_on_top = True
    list_select_related = ["customer", "service", "reservation"]
    inlines = [MaterialConsumptionInline]
    actions = ["mark_in_progress", "mark_ready", "mark_delivered", "export_as_csv"]
    fieldsets = (
        (None, {"fields": ("customer", "vehicle", "service", "reservation")}),
    ...
    def export_as_csv(self, request, queryset):
        ...
        writer.writerow(["ID", "Cliente", "Vehículo", "Servicio", "Estado", "Recibido", "Monto", "Pagado", "Saldo"])
        for obj in queryset.select_related("customer", "vehicle", "service", "reservation"):
            writer.writerow([
                obj.id, obj.customer.name, str(obj.vehicle), obj.service.name,
