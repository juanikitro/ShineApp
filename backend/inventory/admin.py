...
class SupplierAdmin(admin.ModelAdmin):
    list_display = ["name", "category", "contact_name", "phone", "email", "is_active"]
    ...
    fieldsets = (
        (None, {"fields": ("name", "legal_name", "category", "is_active")}),
...
class MaterialAdmin(admin.ModelAdmin):
    list_display = [
        "name", "category", "unit", "stock_quantity_display",
    ...
    save_on_top = True
    fieldsets = (
        (None, {"fields": ("name", "sku", "category", "unit", "presentation", "is_active", "notes")}),
...
class StockMovementAdmin(admin.ModelAdmin):
    list_display = ["id", "movement_type", "occurred_on", "supplier", "total_amount", "affects_cash", "payment_method"]
    list_filter = ["movement_type", "affects_cash", "payment_method", "occurred_on"]
    search_fields = ["supplier__name", "document_number", "notes"]
    date_hierarchy = "occurred_on"
    list_per_page = 25
    ordering = ["-occurred_on", "-id"]
    readonly_fields = ["created_at", "updated_at"]
    autocomplete_fields = ["supplier", "customer", "reservation", "work_order"]
    inlines = [StockMovementLineInline]
    save_on_top = True
    list_select_related = ["supplier"]
    fieldsets = (
        (None, {"fields": ("movement_type", "occurred_on", "supplier", "customer")}),
...
class MaterialPurchaseAdmin(admin.ModelAdmin):
    list_display = ["material", "purchased_at", "quantity", "total_cost", "affects_cash"]
    list_filter = ["affects_cash", "purchased_at"]
...
class MaterialConsumptionAdmin(admin.ModelAdmin):
    list_display = ["material", "work_order", "consumed_at", "quantity", "estimated_unit_cost", "estimated_total_cost"]
    list_filter = ["consumed_at", "material__category"]
    ...
    list_select_related = ["material", "work_order"]


@admin.register(MaterialOpenUnit)
class MaterialOpenUnitAdmin(admin.ModelAdmin):
    list_display = ["material", "status", "opened_at", "finished_at", "stock_quantity_to_decrement"]
...
class ToolAdmin(admin.ModelAdmin):
    list_display = ["name", "quantity", "status", "unit_value", "purchased_at", "is_active"]
